...
# start playing with smarttube
echo "`date` INF `basename $0` try to call smarttube."|tee -a $LOG
...
if [ $rst -eq 0 ] ;then
    ...
    # if there was defined extrasound play it now (unfortunately not working!)
    # start playing with vlc directly
    if [ ! -z $EXTRASOUND ] ; then
	echo "`date` INF try to play extrasound vlc."|tee -a $LOG
	adb -s $DEVICE shell am start -n org.videolan.vlc/.StartActivity $EXTRASOUND
    fi
fi
